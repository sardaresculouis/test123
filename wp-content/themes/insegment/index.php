	...
	<div class="<?php echo esc_attr( $container ); ?>" id="content" tabindex="-1">
		...
		<main class="site-main" id="main">
			...
			<div class="categories-wrap">
				<div class="row">
					<div class="categories col-lg-10 d-flex flex-wrap">

						<?php
						if($cat == ''){
							echo '<span class="category-item-active"><a href="' . get_home_url() . '">'. __("Show All", "insegment_blog") .'</a></span> ';
						} else {
							echo '<span class="category-item"><a href="' . get_home_url() . '">'. __("Show All", "insegment_blog") .'</a></span> ';
						}
						?>
						...
						foreach( $categories as $category ) {
							...
							if( $category->cat_ID == $cat  ){
								echo '<span class="category-item-active">' . sprintf( esc_html__( '%s', 'textdomain' ), $category_link ) . '</span> ';
							} else {
								echo '<span class="category-item">' . sprintf( esc_html__( '%s', 'textdomain' ), $category_link ) . '</span> ';
							}
						}
			...
			<?php if ( have_posts() ) : ?>
				...
				<?php while ( have_posts() ) : the_post(); ?>
					...
					<?php // if ( $i % 3 === 0 ) { echo '</div><div class="blog-post d-flex flex-wrap">'; } ?>
